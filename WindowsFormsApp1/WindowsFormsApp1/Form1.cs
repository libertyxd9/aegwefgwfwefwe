        ...
        void vivoddannix()
        {
            ...
            try
            {
                ...
                string SqlExp = "SELECT dbo.Authors.SurName, dbo.Authors.Name, dbo.Authors.MiddleName, dbo.Authors.NickName, dbo.Books.Name AS Expr1," + 
                    "dbo.Books.PublicationYear, dbo.Books.Amount FROM dbo.Authors INNER JOIN dbo.Books ON dbo.Authors.Id = dbo.Books.AuthorId";
            ...
            switch (comboBox1.SelectedItem.ToString())
            {
                ...
                SqlConnection vivod1 = new SqlConnection(Connection.connectionStr);
                    try
                    {
                        vivod1.Open();
                        string SqlExp = "SELECT dbo.Authors.SurName, dbo.Authors.Name, dbo.Authors.MiddleName, dbo.Authors.NickName, dbo.Books.Name AS Expr1," +
                            "dbo.Books.PublicationYear, dbo.Books.Amount FROM dbo.Authors INNER JOIN dbo.Books ON dbo.Authors.Id = dbo.Books.AuthorId WHERE (dbo.Authors.SurName = N'Брэдбери')";
